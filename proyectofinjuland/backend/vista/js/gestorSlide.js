...
$("#columnasSlide").on("dragover", function(e){
	...
	$("#columnasSlide").css({"background":"url(vista/images/pattern.jpg)"})
...
$("#columnasSlide").on("drop", function(e){
	...
	if(Number(imagenSize) < 2000000 && imagenType == "image/jpeg" || imagenType == "image/png"){
		...
		$.ajax({
			url:"vista/ajax/gestorSlide.php",
			method: "POST",
			data: datos,
			cache: false,
			contentType: false,
			...
			beforeSend: function(){
				...
				$("#columnasSlide").before('<img src="vista/images/status.gif" id="status">');
			...
			success: function(respuesta){
				...
				if(respuesta == 0){
					...
					$("#columnasSlide").before('<div class="alert alert-warning alerta text-center">La imagen es inferior a 1600px * 700px</div>')
